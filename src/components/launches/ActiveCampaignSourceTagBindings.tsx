...
export function ActiveCampaignSourceTagBindings({
  ...
  const [searchByAlias, setSearchByAlias] = useState<Record<string, string>>({});
  ...
  return (
    <div className="space-y-4">
      {bindings.map((binding) => {
        const searchTerm = searchByAlias[binding.alias] ?? "";
        const normalizedSearch = searchTerm.trim().toLowerCase();
        const visibleTags = normalizedSearch
          ? availableTags.filter((tag) =>
              [tag.name, tag.id, tag.description ?? ""]
                .join(" ")
                .toLowerCase()
        ...
        return (
          <div key={binding.alias} className="rounded-xl border border-border/70 bg-background/40 p-4">
            ...
            <Input
              value={searchTerm}
              onChange={(event) =>
                setSearchByAlias((current) => ({
                  ...current,
                  [binding.alias]: event.target.value,
                }))
              }
              placeholder={`Pesquisar tags do ${binding.label}`}
              className="mt-4"
              disabled={disabled}
            />

            <ScrollArea className="mt-3 h-44 rounded-md border border-border/60 px-3 py-2">
