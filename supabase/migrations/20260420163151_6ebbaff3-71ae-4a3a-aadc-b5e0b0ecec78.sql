...
BEGIN
  ...
  RETURN jsonb_build_object(
    'id', updated_row.id, 'name', updated_row.name,
    'project_id', updated_row.project_id, 'slug', updated_row.slug,
    'webhook_secret', updated_row.webhook_secret,
    'ac_api_url', updated_row.ac_api_url, 'ac_api_key', updated_row.ac_api_key,
...
BEGIN
  ...
  DELETE FROM public.uchat_workspaces WHERE launch_id = target_launch_id;

  FOR workspace_item IN SELECT value FROM jsonb_array_elements(sanitized_workspaces) LOOP
    ...
    INSERT INTO public.uchat_workspaces (
      launch_id, project_id, workspace_name, workspace_id, bot_id,
      api_token, welcome_subflow_ns, default_tag_name
    ) VALUES (
      target_launch_id, launch_row.project_id,
      COALESCE(NULLIF(BTRIM(workspace_item->>'workspace_name'), ''), 'Workspace UChat'),
      NULLIF(BTRIM(workspace_item->>'workspace_id'), ''),
      COALESCE(NULLIF(BTRIM(workspace_item->>'bot_id'), ''), NULLIF(BTRIM(workspace_item->>'workspace_id'), '')),
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'workspace_id', workspace.workspace_id, 'api_token', workspace.api_token,
        'welcome_subflow_ns', workspace.welcome_subflow_ns,
        'default_tag_name', workspace.default_tag_name
      ) ORDER BY workspace.created_at ASC
    ), '[]'::jsonb
