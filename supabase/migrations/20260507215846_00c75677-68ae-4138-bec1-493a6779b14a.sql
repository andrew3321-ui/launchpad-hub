-- The ALTER TABLE may have succeeded in the previous migration attempt, so use IF NOT EXISTS
...
drop function if exists public.ensure_hotmart_webhook_settings(uuid);
drop function if exists public.regenerate_hotmart_webhook_token(uuid);

create function public.ensure_hotmart_webhook_settings(target_launch_id uuid)
returns table (
  launch_id uuid,
  enabled boolean,
  webhook_token text,
...
create function public.regenerate_hotmart_webhook_token(target_launch_id uuid)
returns table (
  launch_id uuid,
  enabled boolean,
  webhook_token text,
