-- Google Sheets capture is intentionally looser than platform dedupe:
-- the same phone can represent distinct leads when each row has a different email.
-- Keep phone-only dedupe for rows that do not have an email identity.
